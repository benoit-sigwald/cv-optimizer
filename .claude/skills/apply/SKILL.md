...
2. **`/tailor-cv`** — gap analysis, réécriture, génération DOCX, validation 1 page, copie vers `G:\My Drive\CV\output`.
3. **`/cover-letter`** — lettre dans la langue de l'offre, enregistrée dans `cv_applications`.
4. **`/ats-score`** — score avant/après, mapping keywords, scores enregistrés.

## Sortie finale (une seule synthèse)

- Chemins du CV DOCX + PDF (local et Drive)
